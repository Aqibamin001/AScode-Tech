...
export default function Process() {
  return (
    <section id="process" className="relative bg-cream-200 py-16 sm:py-24 md:py-36 border-t border-cream-300">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 md:px-10">
        <div className="grid grid-cols-12 gap-4 sm:gap-6 md:gap-10 mb-10 sm:mb-16 md:mb-24">
          <div className="col-span-12 md:col-span-5">
            <span className="text-[10px] sm:text-xs uppercase tracking-[0.2em] sm:tracking-[0.25em] text-ink/60">[ 02 — Process ]</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0 border-t border-l border-cream-300">
          {STEPS.map((s) => (
            <div
              key={s.no}
              className="group relative p-6 sm:p-8 md:p-12 border-b border-r border-cream-300 bg-cream-100 hover:bg-cream-50 transition-colors"
            >
              ...
              <h3 className="mt-8 sm:mt-10 md:mt-16 font-display text-3xl sm:text-4xl md:text-6xl tracking-tight">{s.title}</h3>
              <p className="mt-4 sm:mt-6 text-ink/75 max-w-md leading-relaxed text-sm sm:text-base">{s.desc}</p>
            </div>
          ))}
        </div>
      </div>
